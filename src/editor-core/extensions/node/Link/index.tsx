import { getLinkAttributesWithSelectedText, getLinkTitle } from '../../../../utils'
...
import { renderMarkdownLink } from '../resource-shared'
...
export const InlineLinkExtension = Node.create<LinkOptions>({
  ...
  draggable: true,
  selectable: true,
  // This node started from the upstream link implementation, but it now carries
  // product-specific display modes (text/icon/block) and should not be treated
  // as a drop-in replaceable workaround during upgrades.
  ...
  renderMarkdown: (node) => {
    // Local `title` stores display text for the custom link node, not the
    // upstream HTML title attribute semantics introduced later in Tiptap.
    return renderMarkdownLink(node.attrs || {})
  },
